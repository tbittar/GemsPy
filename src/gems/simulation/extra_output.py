...
class VectorizedExtraOutputBuilder(VectorizedBuilderBase[xr.DataArray]):
    ...
    Unlike :class:`~gems.simulation.linopy_linearize.VectorizedLinopyBuilder`,
    this builder operates entirely on concrete ``xr.DataArray`` values (post-solve
    solution data), so nonlinear operations such as products of variables,
    ``floor``, ``ceil``, ``min``, and ``max`` are fully supported.

    The only method defined here is :meth:`variable`, which looks up the
    solved optimal value of a decision variable.
    ...
    var_solution_arrays:
        Mapping from ``(model_key, var_name)`` to a DataArray of solution
        values, with dims in ``{component, time, scenario}`` (or a subset).
    port_arrays:
