...
``VectorizedLinopyBuilder`` overrides a small subset of those methods to add
linopy-specific behaviour (operand-swap in addition, type guards in nonlinear
functions).
"""
...
# ---------------------------------------------------------------------------
# Module-level helper used outside this module (e.g. linopy_problem.py)
# ---------------------------------------------------------------------------
...
class VectorizedBuilderBase(ExpressionVisitor[T_expr]):
    ...
    model_key:
        ...
        Using ``id()`` rather than ``model.id`` ensures two distinct ``Model``
        objects with the same string identifier are never confused.
    model_name:
        Human-readable model identifier (``model.id``), used in error messages.
    param_arrays:
    ...
    def addition(self, node: AdditionNode) -> T_expr:
        ...
        Overridden in :class:`~gems.simulation.linopy_linearize.VectorizedLinopyBuilder`
        to handle mixed DataArray / linopy-type operands.
        """
    ...
    def time_sum(self, node: TimeSumNode) -> T_expr:
        ...
        if from_shift_scalar is not None and to_shift_scalar is not None:
            ...
            for shift in range(from_shift_scalar + 1, to_shift_scalar + 1):
                result = _linopy_add(
                    result, self._apply_time_shift(operand, shift)
                )
            return result  # type: ignore[return-value]
    ...
    # ------------------------------------------------------------------ #
    # Overridden in VectorizedLinopyBuilder to add type guards: these
    # operations cannot be expressed as linear constraints.
